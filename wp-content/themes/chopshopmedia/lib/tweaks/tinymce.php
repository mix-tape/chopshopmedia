...
function csm_tiny_css($wp) {
  $wp .= ',' . get_template_directory_uri().'/assets/styles/styles.css';
  return $wp;
}

add_filter( 'mce_css', 'csm_tiny_css' );
...
add_filter('tiny_mce_before_init', 'csm_mce_remove_unused_formats' );

function csm_mce_remove_unused_formats($init) {
...
function csm_mce_tyles_dropdown( $settings ) {
...
add_filter( 'tiny_mce_before_init', 'csm_mce_tyles_dropdown' );
...
function csm_mce_force_kitch_sink_on(){

  set_user_setting('hidetb', 1);

}

add_action('auth_redirect', 'csm_mce_force_kitch_sink_on');
